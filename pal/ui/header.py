...
class NotificationBell:
    ...
    PANEL_WIDTH  = 380
    PANEL_HEIGHT = 480
    MAX_CARDS    = 20   # máximo de tarjetas visibles en el panel
    ...
    def __init__(
        ...
        # El badge se posiciona encima del botón con place cuando hay notificaciones

        # Bindings
        self._btn.bind("<Button-1>",   self._toggle_panel)
        self._badge.bind("<Button-1>", self._toggle_panel)

        # Registrar como observador del manager
    ...
    def _refresh_badge(self):
        """Actualiza el número del badge según notificaciones no leídas."""
        try:
    ...
    def _on_notifications_changed(self):
        """Callback del observador: actualiza badge y refresca panel si está abierto."""
        try:
    ...
    def _on_focus_out(self, event=None):
        """Cierra el panel si el foco sale de la ventana."""
        try:
            if self._panel_win and self._panel_win.winfo_exists():
                # Pequeño delay para evitar cierre accidental al hacer clic en botones internos
    ...
    def _rebuild_panel_content(self):
        """Limpia y reconstruye todas las tarjetas de notificación."""
    ...
    def _build_card(self, notif):
        """Construye una tarjeta individual para una notificación."""
        ...
        # ── Franja de color izquierda (indicador de prioridad) ────────
        ...
        # Fila 1: icono + título + módulo + timestamp
        ...
        # Botón "Tratar" — solo si tiene modulo_ruta y no está tratada
    ...
    def _on_tratar(self, notif):
        """
        Maneja el clic en el botón "Tratar":
        1. Marca la notificación como tratada en el manager (y BD).
        2. Cierra el panel.
        3. Navega al módulo correspondiente.
        """
        try:
    ...
    def _on_dismiss(self, notif):
        """Descarta la notificación y reconstruye el panel."""
        try:
            self._mgr.dismiss_notification(notif.id)
        except Exception as e:
            print(f"[NotificationBell] Error descartando: {e}")
        # El observador reconstruirá el panel automáticamente
...
def create_header(app):
    """
    Crea el header de la aplicación con degradado azul.
    Incluye el widget NotificationBell si app.notification_manager está disponible.

    El widget de campana se almacena en app.notification_bell para acceso posterior.
    """
