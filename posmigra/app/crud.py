...
from app.utils import hash_password, check_password # Import password utilities
...
def get_all_roles_db(db: Session) -> List[RoleDB]:
    """Fetches all roles from the database."""
    try:
        return db.query(RoleDB).all()
    except SQLAlchemyError as e:
        logger.error(f"Database error fetching all roles: {e}")
        return []

def get_user_permissions(db: Session, user_id: int) -> List[str]:
    """Fetches all permission codes for a given user based on their roles."""
    try:
        ...
        logger.error(f"Database error fetching user permissions for user {user_id}: {e}")
        return []
