...
4. [in_progress] **Migrar la lógica del backend:** Reimplementar o adaptar la lógica de negocio del directorio legacy `pal/` al nuevo framework backend (FastAPI/Flask en `app/`).
    ...
    * [in_progress] **Subtask 4.4:** Implement the backend API endpoints in `app/` that correspond to the migrated logic, using FastAPI or Flask.
        ...
        * [pending] **Subtask 4.4.3:** Implement data validation and error handling for API requests.
    * [pending] **Subtask 4.5:** Integrate security measures (authentication, authorization) into the new API endpoints.
        * [pending] **Subtask 4.5.1:** Apply session verification middleware or decorators to protected endpoints.
        * [pending] **Subtask 4.5.2:** Implement permission checks based on user roles and permissions.
6. [pending] **Integrar Frontend y Backend:** Establecer una comunicación robusta a través de APIs entre el frontend React y el backend FastAPI/Flask para asegurar el flujo de datos y la interactividad del usuario.
    * [pending] **Subtask 6.1:** Define API contracts (request/response formats) between frontend and backend.
    * [pending] **Subtask 6.2:** Implement API calls from React components to the backend endpoints.
    * [pending] **Subtask 6.3:** Handle data fetching, loading states, and error display in the frontend.
7. [pending] **Implementar el Empaquetado de Escritorio (Electron):** Configurar Electron para compilar la aplicación web (frontend React + backend Python) en un ejecutable de escritorio multiplataforma.
    * [pending] **Subtask 7.1:** Set up Electron project configuration.
        * [pending] **Subtask 7.1:** Set up Electron project configuration.
        * [pending] **Subtask 7.2:** Configure Electron to bundle the React frontend and the Python backend.
